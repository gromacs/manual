...
environment variables are listed in the following sections. Most environment
variables function by being set to any non-NULL value. Specific requirements
are described below if other values need to be set.
...
\item   {\tt GMX_TPI_DUMP}: dump all configurations to a {\tt .pdb} file that have an interaction
        energy less than the value set in this environment variable.
...
\item   {\tt LONGFORMAT}: use long float format when printing decimal values.
...
\item   {\tt DISTGCT}: couple distances between two atoms when doing general coupling
        theory processes. The format is two integers, separated by a space.
...
\item   {\tt GMX_ALLOW_CPT_MISMATCH}: runs will not exit if the ensemble set in the {\tt .tpr} 
        file does not match that of the {\tt .cpt} file.
\item   {\tt GMX_CAPACITY}: the maximum capacity of charge groups per processor when using
...
\item   {\tt GMX_DD_ORDER_ZYX}: build domain decomposition cells in the order (z, y, x) rather
        than the default (x, y, z).
\item   {\tt GMX_DISABLE_ACCELERATION}: disables all architecture-specific (SSE2, SSE4, AVX, etc)
        routines.
...
\item   {\tt GMX_FORCEINIT}: shell positions are initiated.
\item   {\tt GMX_FORCE_TABLES}: force the use of tabulated Coulombic and van der Waals interactions.
...
\item   {\tt GMX_IGNORE_FSYNC_FAILURE_ENV}: allow {\tt \normindex{mdrun}} to continue even if
        a file is missing.
\item   {\tt GMX_MAX_THREADS}: sets the maximum number of threads that {\tt \normindex{mdrun}}
        can use.
\item   {\tt GMX_MAXCONSTRWARN}: if set to -1, {\tt \normindex{mdrun}} will not exist if it produces 
...
\item   {\tt VMDDIR}: runtime path of VMD.
