...
\subsection{\swapindex{Flat-bottom}{position restraint}s}
\label{sec:fbposres}
Flat-bottom position restraints can be used to restrain particles to a
certain volume of the simulation box. Within the flat-bottom volume of
the flat-bottom potential, no force acts on the restrained
particle. Only as the particle leaves the boundary of the flat-bottom
volume, a harmonic force pulls the particle
back into the flat-bottom volume. It is possible to apply normal and
flat-bottom position restraints on the same particle (however, only
...
position is depends on the geometry $g$ of the flat-bottom potential.

The following geometries for the flat-bottom potential are supported:\newline
{\bfseries Sphere}: The particle is kept in a sphere of given
...
\eeq
{\bfseries Cylinder}: The particle is kept in a cylinder of given radius
parallel to the z-axis. The force from the flat-bottom potential acts
towards the axis of the cylinder. The z-component of the force is zero.
\beq
 d_g(\ve{r}_i;\ve{R}_i) = \left( (x_i-X_i)^2 + (y_i - Y_i)^2 \right)^{1/2}
\eeq
{\bfseries Layer}: The particle is kept in a layer of a given
thickness. The layer normal can be parallel to the x, y, or
z-axis. The force is acting parallel to the layer normal\\
\beq
...
It is possible to apply two independent flat-bottom position
restrains of different geometry on one particle. For example, applying
a cylinder and a layer in z will keep a particle within a
