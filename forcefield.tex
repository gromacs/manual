...
A coarse-grained force field is not just one specific force field (a set of parameter), but a set of (mostly tabulated) interactions. The coarse grained force field (less degrees of freedom) is designed to reproduce certain properties of an underlying high resolution system (many degrees of freedom), which can be an atomistic system or even an experimental course of setup.
Depending on the properties to reproduce there are different methods to derive such force fields:
\begin{itemize}
\item Conserving free energies
\begin{itemize}
\item MARTINI force-field
\end{itemize}
...
Note that this list is incomplete. There are software packages, based on GROMACS, which 
offer an implementation of the above algorithms, i.e. the \normindex{VOTCA package}~\cite{ruehle2009}, which
can be found under \href{http://www.votca.org}{www.votca.org}, has several coarse-graining 
tutorials using GROMACS on this matter.

An example of a structure-based force field is the \normindex{PLUM force field} to simulate membranes and peptides, which can be found under \href{http://code.google.com/p/plumx/}{code.google.com/p/plumx}. 
