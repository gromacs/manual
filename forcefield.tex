...
A coarse-grainded force field is not just one specific force-field (set of parameter), but a set of (mostly tabulated) interactions, 
made in way, such that the coarse-grained system (less degrees of freedom) reproduces certain properties of underlying high resoultion system 
(many degrees of freedom), which can be an atomistic system or even an experiment.

Depending on the properties to reproduce there are different methods to derrive such force fields:
\begin{itemize}
...
tutortials using GROMACS on this matter.
