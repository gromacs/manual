...
that can in many cases be flatter and lower variance than the softcore
path described above.~\cite{Pham2011} This ``1-1-48'' path is also
implemented in Gromacs. Note that for this path , generally we should
have $0.001 < \alpha < 0.003$,rather than $\alpha \approx 0.5$.
...
\index{dispersion correction} In this section, we derive long-range
corrections due to the use of a cut-off for Lennard-Jones or
Buckingham interactions.  We assume that the cut-off is so long that
the repulsion term can safely be neglected, and therefore only the
dispersion term is taken into account. Due to the nature of the
dispersion interaction, energy and pressure corrections are both
negative. The energy correction is usually small, and could in theory
be computed after the simulation.  In contrast, the pressure
correction is very large for fluids at standard temperature and
pressure and must be computed at each step. Although it is, in
principle, possible to parameterize a force field such that the
pressure is close to 1 bar even without correction, such a method
makes the parameterization dependent on the cut-off and is therefore
undesirable. {\bf Note} that it is not appropriate to use the
long-range correction to the dispersion for a fluid with any nonzero
partial charges without using either a \normindex{reaction field}
