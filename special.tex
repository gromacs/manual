...
altering the topology, solely through the .mdp file, through the use
of the couple-moltype,couple-lambda0, couple-lambda1, and
couple-intramol mdp keywords.  Any molecule type defined identifed in
couple-moltype will automatically have a B state implicitlty
constructed. The molecular parameters that are turned on in the state
are defined by couple-lambda0 and couple-lambda1.  The choices are
...
Starting in 4.6, $lambda$ is a vector of components, with Columbic,
...
'fep-lambdas' is the default lambda array.  All of the other lambda
arrays use the values in this array if they are not specified.  The
previous behavior, where the pathway is controlled by a single
$\lambda$ variable, can be preserved by using only fep-lambdas to
define the pathway.
...
two thirds of the simulation, then you could use something like this:
...
To compute free energies with a multivector $\lambda$ using 
thermodynamic integration, then the equation becomes vector equation:
\beq
