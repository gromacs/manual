...
In comparison to the mixed Quantum-Classical simulation techniques of the previous section, the number of high resolution particles is not fixed, but can vary over the simulation time.

Below we discuss \adress for a coarse-grained and an atomistic representation of the same system, see \figref{adress} for illustration.
The details of implementation described in this section were published in~\cite{Junghans2010,Fritsch2012}.

\begin{figure}
\begin{center}
%\includegraphics[width=\textwidth]{adress_2.eps}
\caption{A schematic illustration of the \adress method for a tetrahedral liquid. In this figure the $\cos^2$ weighting function is shown.} 
\label{fig:adress}
\end{center}
\end{figure}
Every molecule needs a well-defined mapping point in both representations, usually the center of mass,
but any other linear combination of particle coordinates is also sufficient. 
The mapping point is chosen by the virtual site used for the coarse-grained particles.
In this implementation molecules are modeled by charge groups, which actually allows to have multiple mapping point per molecule, which can be useful for bigger molecules like polymers.
...
\end{equation}
where $\alpha$ and $\beta$ label the two molecules and $w_\alpha$, $w_\beta$ are the adaptive weights of the two molecules, which is chosen by the {\tt adress-site} option.
...
The coarse-grained force field is usually derived from the atomistic system by structure-based coarse-graining (see \secref{cg-forcefields}

The important element of this interpolation (see \eqnref{interpolation}) is the adaptive weight function (for illustration see \figref{adress}):
\begin{equation}
w(x)=
...
so essentially \eqnref{interpolation} describes something new for the hybrid region only.
...
The function is 1) continuous, 2) monotonic and 3) having zero derivatives at the boundaries.
...
The interpolation of forces (see \eqnref{interpolation}) can produce inhomogeneities in the density and affect the structure of the system in comparison to a purely explicit simulation.

One way of reducing the inhomogeneities due to the force interpolation is the so-called thermodynamic force.
...
The force can be determined via an interative procedure, which in decribed in detail in the \href{http://code.google.com/p/votca/downloads/list?&q=manual}{manual} of the \normindex{VOTCA package}~\cite{ruehle2009}. Setting the {\tt adress-interface-correction} to {\tt thermoforce} enabled the thermodynamic force correction and {\tt adress-tf-grp-names} defines the energy groups to act on.
