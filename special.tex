...
In this implementation molecules are modeled by charge groups, which actually allows to have multiple mapping points per molecule. This can be useful for bigger molecules like polymers. In that case one has to also extend the AdResS description to bonded interactions~\cite{Praprotnik2011}, which will be implemented into Gromacs in one of the future versions.
...
The coarse-grained force field is usually derived from the atomistic system by structure-based coarse-graining (see \secref{cg-forcefields}). In the toplogy, the coarse-grained interaction is defined by the the mapping point virtual site (both tabulated and analytical potentials can be used). To specify which atoms belong to a coarse-grained representation, energy groups are used.
...
The most important element of this interpolation (see \eqnref{interpolation}) is the adaptive weighting function (for illustration see \figref{adress}):
\begin{equation}
w(x)=
...
The interpolation of forces (see \eqnref{interpolation}) can produce inhomogeneities in the density and affect the structure of the system in the hybrid region.

One way of reducing the inhomogeneities due to the force interpolation is the so-called thermodynamic force (TF)~\cite{Poblete2010}.
...
\end{itemize}
The coarse-grained representation of the interaction is stored as tabulated interaction funtion see \ssecref{userpot}. The convention is to use the $C^{(12)}$  columns with the  $C^{(12)}$- coefficient set to 1. All other columns should be zero. The VOTCA manual has detailed instructions and a tutorial for spc water on how to coarse-grain the interaction using various techniques. 
...
Note that for forcefields which define atomtypes directly in terms of $C_6$ and $C_{12}$ ( like {\tt gmx.ff} ) one can simply set $C_6=0$ and $C_{12}=1$. See section \ssecref{userpot} for more details on tabulated interactions. Since now the water molecule has a virtual site the coordinate file also needs to include that.
{\small
\begin{verbatim}
...
via the \texttt{IMD-group} \texttt{.mdp} option. When \texttt{IMD-group} is empty, the IMD protocol
is disabled and cannot be enabled via the switches in \texttt{mdrun}. To interact with the entire 
system, \texttt{IMD-group} has to be set to \texttt{System}, which is also the default for \texttt{.tpr}
files created with an older version without IMD implementation. When using \texttt{grompp}, a \texttt{.gro} file
to be used with VMD is written out (\texttt{-imd} switch of \texttt{grompp}).
...
an \texttt{mdrun} running locally or on a remote cluster. The port for the connection can be
specified with the \texttt{-imdport} switch of \texttt{mdrun}, 8888 is the default.
...
be altered using the \texttt{-imdfreq} switch. VMD, also allows to increase or reduce the communication
...
\texttt{-imdwait} switch of \texttt{mdrun}. After startup and whenever the client has disconnected, the integration
stops until reconnection of the client.

When \texttt{-imdterm} switch is used, the simulation can be terminated pressing the stop button in VMD. This is disabled by 
default.

Finally, to turn on interaction with the simulation (i.e. pulling from VMD) the \texttt{-imdpull} switch has to be used.

Therefore, a simulation can only be monitored but not influenced from the VMD client when none of \texttt{-imdwait},
\texttt{-imdterm} and \texttt{-imdpull} are set. However, since the IMD protocol requires no authentication, it is not 
advisable to run simulations on a host directly reachable from an insecure environment. Secure shell forwarding of tcp
...
window, hostname and port have to be specified and followed by pressing \textit{Connect}. \textit{Detach Sim} allows disconnecting without 
terminating the simulation while, \textit{Stop Sim} ends the simulation on the next neighboursearching step (if allowed by \texttt{-imdterm}).

The timestep transfer rate allows adjusting the communication frequency between simulation and IMD client. Setting the keep rate loads every n-th frame
into VMD instead of discarding them when a new one is received. The displayed energies are in SI units in contrast to energies displayed from NAMD 
simulations.
