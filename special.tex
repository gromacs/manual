...
Below we discuss this for a coarse-grained and an atomistic representation of the same system, see \figref{adress} for illustration.
The details of implementation described in this section was published in~\cite{Junghans2010,Fritsch2012}.
...
which are a~function of the position of their mapping points. These forces between molecules to their atoms using virtual sites.
...
Spherical and one-dimensional splitting  of the simulation box has been implemented
...
\right.~,
\end{equation}
where $\vec{R}_\mathrm{ct}$ is the center of the explicit zone, which can be fixed or moved in time (e.g. with the center of a molecule).
$\vec{R}_\alpha$ is the mapping point of the $\alpha$th molecule. For the center of mass mapping, it is given by:
\begin{equation}
