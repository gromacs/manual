...
\end{itemize}
The coarse-grained representation of the interaction is stored as tabulated interaction funtion see \ssecref{userpot}. The convention is to use the $C^{(12)}$  columns with the  $C^{(12)}$- coefficient set to 1. All other columns should be zero.\\
...
SOL    8507
\end{verbatim}}
Since now the water molecule has a virtual site the coordinate file also needs to include that.
{\small
\begin{verbatim}
...
As discussed above, the coarse-grained interaction is usually tabulated. This requires the {\tt vdw-type} parameter to be set to {\tt user}.\\
