...
potential forms support both a $A$ state and a $B$ state. Whenever both
...
{\tt couple-intramol} mdp keywords.  Any molecule type defined identified in
{\tt couple-moltype} will automatically have a B state implicitly
constructed. The molecular parameters that are turned on in the state
are defined by {\tt couple-lambda0} and {\tt couple-lambda1}.  The choices are
'q','vdw', and 'vdw-q'; these indicate the parameters that are turned
on in the 0 or 1 state.
...
Starting in 4.6, $lambda$ is a vector of components, with Coulumbic,
...
sample all the way from interacting to noninteracting states in the
...
the van der Walls, but changing the restraints throughout the first
...
The coarse-grained force field is usually derived from the atomistic system by structure-based coarse-graining (see \secref{cg-forcefields}). In the topology, the coarse-grained interaction is defined by the the mapping point virtual site (both tabulated and analytical potentials can be used). To specify which atoms belong to a coarse-grained representation, energy groups are used.
...
SOL    8507
\end{verbatim}}
The $\sigma$ and $\epsilon$ values coorespond to $C_6=1$ and $C_{12}=1$ and thus the table file should contain the coarse-grained interaction in either the $C_6$ or $C_{12}$ column. In the example the OPLS forcefield is used where $\sigma$ and $\epsilon$ are specified.
...
only a subset of more advanced CUDA-enabled GPUs (for detailed list see section 
...
\begin{enumerate}
    ...
    {\small{\tt export LD_LIBRARY_PATH=path_to_gromacs/openmm/lib:\$LD_LIBRARY_PATH}}\\
...
% LocalWords:  itp sitesn atomtypes ff csg ndx Tesla CHARMM AA
% LocalWords:  CMAP nocmap fators Monte performant lib LD DIR
